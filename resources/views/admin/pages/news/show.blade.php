    ...
    <div class="card-panel">
        <div class="row">
            ...
            <div class="col s12 m5 quick-action-btns display-flex justify-content-end align-items-center pt-2">
                <a href="{{locale_route('news.edit',$news->id)}}" class="btn-small indigo">
                    @lang('admin.edit')
                </a>
                <a class="btn-small -settings waves-effect -light -btn right ml-3"
                   href="{{locale_route('news.destroy',$news->id)}}"
                        ...
                        <div id="cat-{{$locale}}"
                            ...
                            <table class="striped">
                                ...
                                <tr>
                                    ...
                                    <td>{{$news->translate($locale)->title ?? ''}}</td>
                                </tr>
                                <tr>
                                    <td>@lang('admin.date'):</td>
                                    <td>{{$news->translate($locale)->date ?? ''}}</td>
                                </tr>
                                <tr>
                                    <td>@lang('admin.description'):</td>
                                    <td>{{$news->translate($locale)->description ?? ''}}</td>
                                </tr>
                                <tr>
                                    <td>@lang('admin.short_description'):</td>
                                    <td>{{$news->translate($locale)->short_description ?? ''}}</td>
                                </tr>
                                <tr>
                                    <td>@lang('admin.meta_title'):</td>
                                    <td>{{$news->translate($locale)->meta_title ?? ''}}</td>
                                </tr>
                                <tr>
                                    <td>@lang('admin.meta_description'):</td>
                                    <td>{{$news->translate($locale)->meta_description ?? ''}}</td>
                                </tr>
                                <tr>
                                    <td>@lang('admin.meta_keywords'):</td>
                                    <td>{{$news->translate($locale)->meta_keywords ?? ''}}</td>
                                </tr>
                                </tbody>
                            </table>
                        </div>
